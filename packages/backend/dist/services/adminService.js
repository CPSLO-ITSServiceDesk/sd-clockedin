...
exports.adminService = {
    ...
    async create(payload) {
        ...
        if (existing) {
            throw new errorHandler_1.HttpError(409, 'An admin with this email already exists');
        }
        const insertPayload = {
    ...
    async remove(id) {
        const existing = await this.getById(id);
        if (!existing) {
            throw new errorHandler_1.HttpError(404, 'Admin not found');
        }
        const { error } = await supabase_1.supabase.from('admins').delete().eq('id', id);
        if (error)
            throw new errorHandler_1.HttpError(500, error.message);
    },
    async authorize(email, name) {
        const normalized = normalizeEmail(email);
        if (!normalized) {
